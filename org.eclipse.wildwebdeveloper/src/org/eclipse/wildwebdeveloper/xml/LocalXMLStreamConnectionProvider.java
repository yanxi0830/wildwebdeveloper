 ...
 */
public class LocalXMLStreamConnectionProvider extends LocalStreamConnectionProvider2 {

	public LocalXMLStreamConnectionProvider() {
		super(Activator.getDefault().getLog(), Activator.PLUGIN_ID);
	}

	private static final String LANGUAGE_SERVER_JAR_ENTRY_NAME = "server/xml-languageserver-all.jar"; //$NON-NLS-1$
...
//		}
		...
		try {
			launchedFuture = XMLServerLauncher.launch(clientToServerStream, serverToClientStream);
...
//				}
	...
	private URL[] getFreemarkerLanguageServerClassPath() {
		...
		if (bundle == null) {
			throw new RuntimeException("Bundle " + Activator.PLUGIN_ID + " not found"); //$NON-NLS-1$
		}

		URL languageServerJarURL = bundle.getEntry(LANGUAGE_SERVER_JAR_ENTRY_NAME);
		if (languageServerJarURL == null) {
			throw new RuntimeException(
					"Entity " + LANGUAGE_SERVER_JAR_ENTRY_NAME + " not found in bundle " + Activator.PLUGIN_ID); //$NON-NLS-1$
		}
